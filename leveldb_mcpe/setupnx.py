...
if (not os.system('ls -R build/*/leveldb_mcpe.so 1> /dev/null 2>&1')) or os.path.exists(os.path.join(destpath, 'leveldb_mcpe.so')):
    a = input('The library is already present. Do you want to recompile [y/N]?')
    if a and a in 'yY':
...
if install_boost is None:  # Shall not happen...
    ...
    a = input('Do you want to (re)install it [y/N] ?')
    if a and a in 'yY':
        install_boost = True
...
if (not os.path.exists(mcpeso_dir)) or (not 'libleveldb.so' in os.listdir(mcpeso_dir)):
    ...
    a = input("Mojang's leveldb is already built. Rebuild [y/N] ?")
    if a and a in 'yY':
        build_leveldb = True
    else:
        build_leveldb = False
...
if build_leveldb:
    if os.path.exists('leveldb-mcpe') and os.listdir('leveldb-mcpe') != []:
        a = input("Mojang's leveldb-mcpe source directory already exists. Replace it (reextract) [y/N] ?")
